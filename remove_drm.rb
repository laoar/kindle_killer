...
class MobiBook
    ...
    def process_book(pidlist)
        ...
        if @crypto_type == 1
            ...
            found_key, pid = parse_drm(@sect[drm_ptr...drm_ptr+drm_size], drm_count, goodpids)

            # kill the drm keys
            patch_section(0, "\0" * drm_size, drm_ptr)            
    ...
    def initialize(file_name)
        ...
        if (exth.length >= 4) and (exth[0...4] == 'EXTH') #sizeof
            ...
            for i in 0...nitems do
                ...
                if type == 401 &&  size == 9
                    patch_section(0, "\144", 16 + @mobi_length + pos + 8)
                elsif type == 404 && size == 9 
                    patch_section(0, "\0", 16 + @mobi_length + pos + 8)
                end
                pos += size
            end
        end
